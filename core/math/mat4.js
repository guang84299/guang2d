...
(function(g) {
     ...
     * | 3   7  11  15 |
     * </p>
     * @param {g.math.Matrix4} [mat4]
     */
    g.math.Matrix4 = function (mat4) {
        if(mat4 && mat4.mat){
    ...
    g.kmMat4 = g.math.Matrix4;
    var proto = g.math.Matrix4.prototype;

    /**
     * Fills a g.math.Matrix4 structure with the values from a 16 element array of floats
     * @param {Array} scalarArr
     */
    proto.fill = function(scalarArr){        //g.kmMat4Fill
        var mat = this.mat;
        for(var i = 0; i < 16; i++){
            mat[i] = scalarArr[i];
        }
     ...
     */
    g.kmMat4Identity = function (pOut) {
        var mat = pOut.mat;
    ...
    g.math.Matrix4._gaussj = function (a, b) {
    ...
    var identityMatrix = new g.math.Matrix4().identity();
    /**
     ...
     */
    g.kmMat4Inverse = function (pOut, pM) {
        var inv = new g.math.Matrix4(pM);
        var tmp = new g.math.Matrix4(identityMatrix);
        if (g.math.Matrix4._gaussj(inv, tmp) === false)
            return null;
        pOut.assignFrom(inv);
        return pOut;
    };
     ...
     * @returns {g.math.Matrix4} Returns null if there is no inverse, else returns a new inverse matrix object
     */
    proto.inverse = function(){    //g.kmMat4Inverse
        var inv = new g.math.Matrix4(this);
        var tmp = new g.math.Matrix4(identityMatrix);
        if (g.math.Matrix4._gaussj(inv, tmp) === false)
            return null;
        return inv;
     ...
     */
    proto.isIdentity = function () {       // g.kmMat4IsIdentity
        var mat = this.mat;
     ...
     */
    proto.transpose = function() {        // g.kmMat4Transpose
        var mat = this.mat;
     ...
     */
    g.kmMat4Multiply = function (pOut, pM1, pM2) {
    ...
    g.getMat4MultiplyValue = function (pM1, pM2) {
     ...
     */
    g.kmMat4Assign = function (pOut, pIn) {
        if (pOut === pIn) {
            g.log("g.kmMat4Assign(): pOut equals pIn");
            return pOut;
        }
    ...
    proto.assignFrom = function(mat4) {
        if (this === mat4) {
            g.log("g.mat.Matrix4.assignFrom(): mat4 equals current matrix");
            return this;
        }
    ...
    proto.equals = function(mat4) {
        if (this === mat4) {
            g.log("g.kmMat4AreEqual(): pMat1 and pMat2 are same object.");
            return true;
        }
        var matA = this.mat, matB = mat4.mat, EPSILON = g.math.EPSILON;
        for (var i = 0; i < 16; i++) {
     ...
     * @param {g.math.Matrix4} [matrix]
     * @returns {g.math.Matrix4}
     */
    g.math.Matrix4.createByRotationX = function(radians, matrix) {    //g.kmMat4RotationX
        /*
         |  1  0       0       0 |
         M = |  0  cos(A) -sin(A)  0 |
         |  0  sin(A)  cos(A)  0 |
         |  0  0       0       1 |
         */
        matrix = matrix || new g.math.Matrix4();
        var mat = matrix.mat;
        mat[0] = 1.0;
     ...
     * @returns {*}
     */
    g.math.Matrix4.createByRotationY = function(radians, matrix) {      // g.kmMat4RotationY
        /*
         |  cos(A)  0   sin(A)  0 |
         M = |  0       1   0       0 |
         | -sin(A)  0   cos(A)  0 |
     ...
     * @param {Number} radians
     * @param {g.math.Matrix4} matrix
     * @return {g.math.Matrix4}
     */
    g.math.Matrix4.createByRotationZ = function(radians, matrix){    // g.kmMat4RotationZ
        /*
         |  cos(A)  -sin(A)   0   0 |
         M = |  sin(A)   cos(A)   0   0 |
         |  0        0        1   0 |
     ...
     * @param {Number} yaw
     * @param {Number} roll
     * @param {g.math.Matrix4} [matrix] if matrix is undefined, creates a new matrix.
     * @returns {g.math.Matrix4}
     */
    g.math.Matrix4.createByPitchYawRoll = function(pitch, yaw, roll, matrix) {
        matrix = matrix || new g.math.Matrix4();
     ...
     * @returns {g.math.Matrix4}
     */
    g.math.Matrix4.createByQuaternion = function(quaternion, matrix) {
        matrix = matrix || new g.math.Matrix4();
        var mat = matrix.mat;
     ...
     * @param {g.math.Matrix3} rotation
     * @param {g.math.Vec3} translation
     * @param {g.math.Matrix4} [matrix] if matrix is undefined, creates a new matrix.
     * @returns {g.math.Matrix4}
     */
    g.math.Matrix4.createByRotationTranslation = function(rotation, translation, matrix) {
        matrix = matrix || new g.math.Matrix4();
     ...
     * @returns {g.math.Matrix4}
     */
    g.math.Matrix4.createByScale = function(x, y, z, matrix) {  //g.kmMat4Scaling
        matrix = matrix || new g.math.Matrix4();
        var mat = matrix.mat;
        mat[0] = x;
        mat[5] = y;
     ...
     */
    g.kmMat4Translation = function (pOut, x, y, z) {
     ...
     * @returns {g.math.Matrix4}
     */
    g.math.Matrix4.createByTranslation = function(x, y, z, matrix){       //g.kmMat4Translation
        matrix = matrix || new g.math.Matrix4();
        matrix.identity();
        matrix.mat[12] = x;
        matrix.mat[13] = y;
    ...
    proto.getUpVec3 = function() {
        var mat = this.mat;
        var ret = new g.math.Vec3(mat[4],mat[5], mat[6]);
    ...
    proto.getRightVec3 = function(){
        var mat = this.mat;
        var ret = new g.math.Vec3(mat[0],mat[1], mat[2]);
        return ret.normalize();
    };
    ...
    proto.getForwardVec3 = function() {
        var mat = this.mat;
        var ret = new g.math.Vec3(mat[8],mat[9], mat[10]);
        return ret.normalize();
    };
     ...
     */
    g.kmMat4PerspectiveProjection = function (pOut, fovY, aspect, zNear, zFar) {
        var r = g.degreesToRadians(fovY / 2);
     ...
     * @param {Number} zNear
     * @param {Number} zFar
     * @returns {g.math.Matrix4|Null}
     */
    g.math.Matrix4.createPerspectiveProjection = function(fovY, aspect, zNear, zFar){
        var r = g.degreesToRadians(fovY / 2), deltaZ = zFar - zNear;
        var s = Math.sin(r);
        ...
        //cos(r) / sin(r) = cot(r)
        var cotangent = Math.cos(r) / s;
        var matrix = new g.math.Matrix4(), mat = matrix.mat;
        matrix.identity();
    ...
    g.kmMat4OrthographicProjection = function (pOut, left, right, bottom, top, nearVal, farVal) {
        pOut.identity();
     ...
     * @returns {g.math.Matrix4}
     */
    g.math.Matrix4.createOrthographicProjection = function (left, right, bottom, top, nearVal, farVal) {
        var matrix = new g.math.Matrix4(), mat = matrix.mat;
        matrix.identity();
        mat[0] = 2 / (right - left);
        mat[5] = 2 / (top - bottom);
     ...
     */
    g.kmMat4LookAt = function (pOut, pEye, pCenter, pUp) {
        var f = new g.math.Vec3(pCenter), up = new g.math.Vec3(pUp);
        f.subtract(pEye);
        f.normalize();
        up.normalize();

        var s = new g.math.Vec3(f);
        s.cross(up);
        s.normalize();

        var u = new g.math.Vec3(s);
        u.cross(f);
        s.normalize();
        ...
        var translate = g.math.Matrix4.createByTranslation(-pEye.x, -pEye.y, -pEye.z);
    ...
    proto.lookAt = function(eyeVec, centerVec, upVec) {
        var f = new g.math.Vec3(centerVec), up = new g.math.Vec3(upVec), mat = this.mat;
        f.subtract(eyeVec);
        f.normalize();
        up.normalize();
        ...
        tempMatrix = g.math.Matrix4.createByTranslation(-eyeVec.x, -eyeVec.y, -eyeVec.z, tempMatrix);
     ...
     * pOut is returned.
     */
    g.kmMat4RotationAxisAngle = function (pOut, axis, radians) {
        var rcos = Math.cos(radians), rsin = Math.sin(radians);

        var normalizedAxis = new g.math.Vec3(axis);
     ...
     * @returns {g.math.Matrix4}
     */
    g.math.Matrix4.createByAxisAndAngle = function(axis, radians, matrix) {
        matrix = matrix || new g.math.Matrix4();
     ...
     * @returns {g.math.Matrix3}
     */
    proto.extractRotation = function(){
        var matrix = new g.math.Matrix3(), mat4 = this.mat, mat3 = matrix.mat;
        mat3[0] = mat4[0];
        mat3[1] = mat4[1];
        mat3[2] = mat4[2];
    ...
    proto.extractPlane = function(planeType) {
        var plane = new g.math.Plane(), mat = this.mat;
        switch (planeType) {
            case g.math.Plane.RIGHT:
                plane.a = mat[3] - mat[0];
                plane.b = mat[7] - mat[4];
                plane.c = mat[11] - mat[8];
                plane.d = mat[15] - mat[12];
                break;
            case g.math.Plane.LEFT:
                plane.a = mat[3] + mat[0];
                plane.b = mat[7] + mat[4];
                plane.c = mat[11] + mat[8];
                plane.d = mat[15] + mat[12];
                break;
            case g.math.Plane.BOTTOM:
                plane.a = mat[3] + mat[1];
                plane.b = mat[7] + mat[5];
                plane.c = mat[11] + mat[9];
                plane.d = mat[15] + mat[13];
                break;
            case g.math.Plane.TOP:
                plane.a = mat[3] - mat[1];
                plane.b = mat[7] - mat[5];
                plane.c = mat[11] - mat[9];
                plane.d = mat[15] - mat[13];
                break;
            case g.math.Plane.FAR:
                plane.a = mat[3] - mat[2];
                plane.b = mat[7] - mat[6];
                plane.c = mat[11] - mat[10];
                plane.d = mat[15] - mat[14];
                break;
            case g.math.Plane.NEAR:
                plane.a = mat[3] + mat[2];
                plane.b = mat[7] + mat[6];
                plane.c = mat[11] + mat[10];
                plane.d = mat[15] + mat[14];
                break;
            default:
                g.log("g.math.Matrix4.extractPlane: Invalid plane index");
                break;
        }
    ...
    proto.toAxisAndAngle = function() {
        ...
        var temp = g.math.Quaternion.rotationMatrix(rotation);

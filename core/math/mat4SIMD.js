...
(function(g) {
    ...
    g.kmMat4InverseSIMD = function (pOut, pM) {
        pOut = pM.inverseSIMD();
        return pOut;
    };

    proto.inverseSIMD = function(){
        var inv = new g.math.Matrix4();
        var src = this.mat;
        var dest = inv.mat;
    ...
    g.kmMat4MultiplySIMD = function (pOut, pM1, pM2) {
        pOut = new g.math.Matrix4(pM1);
    ...
    g.getMat4MultiplyValueSIMD = function (pM1, pM2) {
        var mat = new g.math.Matrix4(pM1);
        return mat.multiplySIMD(pM2);
    };

    g.kmMat4AssignSIMD = function (pOut, pIn) {
        if(pOut == pIn) {
            g.log("g.kmMat4Assign(): pOut equals pIn");//TODO: ADD SIMD?
            return pOut;
        }
    ...
    proto.assignFromSIMD = function (mat4) {
        if(this == mat4) {
            g.log("g.mat.Matrix4.assignFrom(): mat4 equals current matrix");//TODO: ADD SIMD?
            return this;
        }
    ...
    proto.equalsSIMD = function (mat4) {
        ...
        var epsilon = SIMD.float32x4.splat(g.math.EPSILON);
    ...
    g.kmMat4LookAtSIMD = function (pOut, pEye, pCenter, pUp) {
    ...
    proto.lookAtSIMD = function(eyeVec, centerVec, upVec) {
        ...
        // g.kmVec3Subtract(f, pCenter, pEye);
        var f = SIMD.float32x4.sub(center, eye);
        // g.kmVec3Normalize(f, f);
        ...
        // g.kmMat4Translation(translate, -pEye.x, -pEye.y, -pEye.z);
        ...
        // g.kmMat4Multiply(pOut, pOut, translate);
        SIMD.float32x4.store(out, 0, SIMD.float32x4.add(
            SIMD.float32x4.mul(
                SIMD.float32x4.swizzle(b0, 0, 0, 0, 0), a0),
            SIMD.float32x4.add(
